...
API_URL = os.getenv("API_URL", "http://fastapi:8888")
...
st.session_state.user_name = st.sidebar.text_input(
    "Имя", value=st.session_state.user_name
)
st.session_state.user_surname = st.sidebar.text_input(
    "Фамилия", value=st.session_state.user_surname
)
st.session_state.user_position = st.sidebar.text_input(
    "Должность", value=st.session_state.user_position
)
st.session_state.user_contact = st.sidebar.text_input(
    "Контакт (TG/телефон)", value=st.session_state.user_contact
)
...
if st.session_state.logged_in:
    st.sidebar.markdown("Статус: 🟢 Вошел в систему")
    if st.session_state.user_name or st.session_state.user_surname:
        st.sidebar.markdown(
            f"Текущий пользователь: {st.session_state.user_name} "
            f"{st.session_state.user_surname}"
        )
    ...
    st.sidebar.markdown("Статус: 🔴 Не в системе")
...
if data is None:
    st.warning(
        "Не удалось загрузить данные. Убедитесь, что FastAPI запущен."
    )
    st.stop()
...
with st.sidebar.form("add_form"):
    ...
    if submitted:
        ...
        try:
            resp = requests.post(f"{API_URL}/records", json=payload, timeout=5)
    ...
    if delete_submitted:
        try:
            resp = requests.delete(
                f"{API_URL}/records/{delete_id}", timeout=5
            )
